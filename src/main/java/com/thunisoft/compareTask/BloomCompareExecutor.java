...
/**
 * Description:
 * Datetime:    2021/1/7   14:05
 * Author:   zhangliujie
 */
@Slf4j
public class BloomCompareExecutor implements Runnable{
    ...
    BloomCompareExecutor(String sqlLeft, String sqlRight, List<String> columns, CountDownLatch countDownLatch) {
    ...
    public void run() {
        ...
        log.info(sqlLeft);
        try {
            BloomFilter<String> pkFilter = BloomFilter.create(Funnels.stringFunnel(Charsets.UTF_8), 200000, 0.01);
            BloomFilter<String> md5Filter = BloomFilter.create(Funnels.stringFunnel(Charsets.UTF_8), 200000, 0.01);
            try {
            ...
            try{
                ...
                while (rs.next()) {
                    ...
                    String md5 = rs.getString(columns.size() + 1);
                    pkFilter.put(pkKey);
                    md5Filter.put(pkKey+md5);
            ...
            try{
                ...
                while (rs2.next()) {
                    ...
                    if (pkFilter.mightContain(pkKey)) {
                        //内容差异
                        String md5Right = rs2.getString(columns.size() + 1);
                        if (!md5Filter.mightContain(pkKey + md5Right)) {
                            Map<String, Object> row = new HashMap<>();
                            row.put(columns.get(0), pkKey);
